...
CREATE TABLE articles (
    ...
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP, -- Record creation time
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, -- Record update time
    FOREIGN KEY (author_id) REFERENCES users(id) ON DELETE CASCADE, -- Relation with users
    FOREIGN KEY (category_id) REFERENCES categories(id) ON DELETE CASCADE -- Relation with categories
);
